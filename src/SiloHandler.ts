...
export function handleRemoveDeposit(event: RemoveDeposit): void {
    ...
    removeDepositFromSiloAsset(event.address, event.params.token, beanstalk.lastSeason, withdrawnBDV, event.params.amount, event.block.timestamp, event.block.number)
    ...
    removeDepositFromSiloAsset(event.params.account, event.params.token, beanstalk.lastSeason, withdrawnBDV, event.params.amount, event.block.timestamp, event.block.number)
...
export function handleRemoveDeposits(event: RemoveDeposits): void {
    ...
    for (let i = 0; i < event.params.seasons.length; i++) {
        ...
        // Update protocol totals
        removeDepositFromSilo(event.address, beanstalk.lastSeason, withdrawnBDV, event.block.timestamp, event.block.number)
        removeDepositFromSiloAsset(event.address, event.params.token, beanstalk.lastSeason, withdrawnBDV, event.params.amounts[i], event.block.timestamp, event.block.number)

        // Update farmer totals
        removeDepositFromSilo(event.params.account, beanstalk.lastSeason, withdrawnBDV, event.block.timestamp, event.block.number)
        removeDepositFromSiloAsset(event.params.account, event.params.token, beanstalk.lastSeason, withdrawnBDV, event.params.amounts[i], event.block.timestamp, event.block.number)
...
export function updateStalkWithCalls(season: i32, timestamp: BigInt, blockNumber: BigInt): void {
    ...
    for (let i = 0; i < beanstalk.farmersToUpdate.length; i++) {
        ...
        let silo = loadSilo(account)
        updateStalkBalances(account, season, beanstalk_call.balanceOfStalk(account).minus(silo.stalk), beanstalk_call.balanceOfRoots(account).minus(silo.roots), timestamp, blockNumber)
